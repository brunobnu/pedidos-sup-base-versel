  ...
  return `
    ...
    <section class="panel">
      <div class="section-title">
        <div><h2>Migrar base local</h2><p class="muted">Use para levar para o portal online os clientes, cadastros, status e comentários que já foram tratados localmente.</p></div>
        <div class="toolbar">
          <button class="btn" data-action="export-local-base">Exportar base local</button>
          <button class="btn primary" data-action="import-local-base">Importar base local</button>
          <input type="file" id="localBaseInput" accept=".json,application/json" hidden />
        </div>
      </div>
      <div class="dropzone">
...
function bind() {
  ...
  document.querySelector("[data-action='export-local-base']")?.addEventListener("click", exportLocalBase);
  document.querySelector("[data-action='import-local-base']")?.addEventListener("click", () => document.getElementById("localBaseInput")?.click());
  document.getElementById("localBaseInput")?.addEventListener("change", importLocalBase);
...
function exportLocalBase() {
  ...
  const payload = {
    type: "pedidos-clientes-local-backup",
    version: window.APP_VERSION || "local",
    exportedAt: new Date().toISOString(),
    clients: state.clients,
    settings: { ...state.settings, method: state.method, forecastMethod: state.method },
    ai: state.ai,
  };
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = `base-pedidos-clientes-${new Date().toISOString().slice(0, 10)}.json`;
  link.click();
  URL.revokeObjectURL(link.href);
}

async function importLocalBase(event) {
  if (!requirePermission("import")) return;
  ...
  try {
    ...
    if (!Array.isArray(importedClients) || !importedClients.length) throw new Error("Arquivo sem clientes validos.");
    if (!confirm(`Importar ${importedClients.length} clientes da base local? Clientes com mesmo nome serao atualizados e enviados ao banco online.`)) return;

    const merged = mergeLocalBaseClients(state.clients, importedClients);
    state.clients = merged.clients;
    state.selectedId = merged.firstImportedId || state.clients[0]?.id || null;
    if (parsed.settings) {
      state.settings = { ...state.settings, ...parsed.settings };
      state.method = parsed.settings.method || parsed.settings.forecastMethod || state.method;
      saveSettings({ ...state.settings, method: state.method, forecastMethod: state.method });
    }
    if (parsed.ai?.analyses?.length) {
      state.ai = {
        credits: Number.isFinite(Number(parsed.ai.credits)) ? Number(parsed.ai.credits) : state.ai.credits,
        analyses: mergeAnalyses(state.ai.analyses, parsed.ai.analyses),
      };
      saveAiState();
    }
    saveClients(state.clients);
    if (location.protocol !== "file:") await syncRemoteClients(state.clients);
    state.importMessages = [{ ok: true, text: `${importedClients.length} clientes importados da base local. Atualize a pagina para confirmar a persistencia.` }];
  } catch (error) {
    state.importMessages = [{ ok: false, text: `Base local: ${error.message || "arquivo invalido."}` }];
  } finally {
    event.target.value = "";
    render();
  }
...
function mergeLocalBaseClients(currentClients, importedClients) {
  const byKey = new Map();
  currentClients.forEach((client) => {
    byKey.set(clientKey(client.name), cloneClient(client));
  });
  let firstImportedId = null;
  importedClients.forEach((rawClient) => {
    const incoming = cloneClient(rawClient);
    if (!incoming.name) return;
    incoming.records = (incoming.records || []).map((record) => ({
      id: record.id || crypto.randomUUID(),
      competencia: record.competencia,
      quantity: Number(record.quantity) || 0,
      source: record.source || "importado",
      observation: record.observation || "",
    })).filter((record) => /^\d{4}-\d{2}$/.test(record.competencia));
    incoming.comments = incoming.comments || [];
    const keyName = clientKey(incoming.name);
    const found = byKey.get(keyName);
    if (!found) {
      incoming.id ||= crypto.randomUUID();
      incoming.active = incoming.active !== false;
      incoming.dashboardActive = incoming.dashboardActive !== false;
      byKey.set(keyName, incoming);
      firstImportedId ||= incoming.id;
      return;
    }
    const id = found.id || incoming.id || crypto.randomUUID();
    const merged = {
      ...found,
      ...incoming,
      id,
      name: preferredClientName(found.name, incoming.name),
      cnpj: incoming.cnpj || found.cnpj || "",
      erp: incoming.erp || found.erp || "",
      segment: incoming.segment || found.segment || "",
      owner: incoming.owner || found.owner || "",
      notes: incoming.notes || found.notes || "",
      active: incoming.active !== false,
      dashboardActive: incoming.dashboardActive !== false,
      records: mergeRecords(found.records || [], incoming.records || []),
      comments: mergeComments(found.comments || [], incoming.comments || []),
      importedAt: incoming.importedAt || found.importedAt || new Date().toISOString(),
    };
    byKey.set(keyName, merged);
    firstImportedId ||= id;
  });
  return { clients: [...byKey.values()], firstImportedId };
}
...
function mergeAnalyses(current, incoming) {
  const byId = new Map();
  [...current, ...incoming].forEach((analysis) => {
    const id = analysis.id || crypto.randomUUID();
    byId.set(id, { ...analysis, id });
  });
  return [...byId.values()].sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")));
}
